...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const requiredFields = ['order_id', 'customer', 'items', 'total'];
        ...
        const customerRequiredFields = ['firstName', 'pinCode'];
        ...
        if (missingCustomerFields.length > 0) {
            return {
                ...
                body: JSON.stringify({
                    success: false,
                    error: `Missing customer fields: ${missingCustomerFields.join(', ')}`
                })
            };
        }
        ...
        console.error('❌ Error in shiprocket-create-order function:', error);
        
        // Parse specific error messages
        let errorMessage = error.message;
        let statusCode = 500;
        
        if (error.message.includes('Serviceability check failed')) {
            statusCode = 400;
            errorMessage = 'Service not available for this pincode';
        } else if (error.message.includes('Authentication failed')) {
            statusCode = 401;
            errorMessage = 'Shiprocket authentication failed - check credentials';
        } else if (error.message.includes('Required field missing')) {
            statusCode = 400;
            errorMessage = 'Missing required shipping information';
        }
